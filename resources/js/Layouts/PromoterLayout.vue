    ...
    <div>
        ...
        <div class="flex h-screen bg-card">
            <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'"
                ...
                <div>
                    <div class="p-4">
                        <Link :href="route('promoter.index')" class="flex items-center">
                            <div class="flex items-center space-x-2">
                                <ApplicationLogo class="block h-12 p-2 rounded-md w-auto bg-primary text-white" />
                                <span
                                    class="flex justify-center text-pri font-bold tracking-wide text-2xl uppercase">SoundScape</span>
                            </div>
                        </Link>
                    </div>
                    <nav class="py-2 flex-1 flex flex-col space-y-2 px-2 overflow-hidden uppercase">
                        <NavLink :href="route('promoter.index')" :active="route().current('promoter.index')">
                            <span class="w-full">Dashboard</span>
                            <DashboardCurrent :active="route().current('promoter.index')" />
                        </NavLink>
                        <NavLink :href="route('promoter.concert.index')"
                            :active="route().current('promoter.concert.*')">
                            <span class="w-full">Concerts</span>
                            <DashboardCurrent :active="route().current('promoter.concert.*')" />
                        </NavLink>
                    </nav>
                </div>
                <div class="p-2 mt-auto">
                    <Link :href="route('promoter.profile')"
