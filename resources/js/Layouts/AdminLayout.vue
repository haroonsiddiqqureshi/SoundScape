    ...
    <div>
        ...
        <div class="flex h-screen bg-card">
                    ...
                    <div class="p-4">
                        <Link :href="route('admin.dashboard')" class="flex items-center">
                            <div class="flex items-center space-x-2">
                                ...
                                <span
                                    class="flex justify-center text-primary font-bold tracking-wide text-2xl uppercase">SoundScape</span>
                            </div>
                        </Link>
                    </div>
                    <nav class="py-2 flex-1 flex flex-col space-y-2 px-2 overflow-hidden uppercase">
                        <NavLink :href="route('admin.dashboard')" :active="route().current('admin.dashboard')">
                            <span class="w-full">Dashboard</span>
                            <DashboardCurrent :active="route().current('admin.dashboard')" />
                        </NavLink>
                        <NavLink :href="route('admin.highlight.index')" :active="route().current('admin.highlight.*')">
                            <span class="w-full">Highlights</span>
                            <DashboardCurrent :active="route().current('admin.highlight.*')" />
                        </NavLink>
                        <NavLink :href="route('admin.concert.index')" :active="route().current('admin.concert.*')">
                            <span class="w-full">Concerts</span>
                            <DashboardCurrent :active="route().current('admin.concert.*')" />
                        </NavLink>
                        <NavLink :href="route('admin.artist.index')" :active="route().current('admin.artist.*')">
                            <span class="w-full">Artists</span>
                            <DashboardCurrent :active="route().current('admin.artist.*')" />
                        </NavLink>
                        <NavLink :href="route('admin.promoter.index')" :active="route().current('admin.promoter.*')">
                            <span class="w-full">Promoters</span>
                            <DashboardCurrent :active="route().current('admin.promoter.*')
                                " />
                        </NavLink>
                        <NavLink :href="route('admin.user.index')" :active="route().current('admin.user.*')">
                            <span class="w-full">Users</span>
                            <DashboardCurrent :active="route().current('admin.user.*')" />
                        </NavLink>
                    </nav>
                </div>
            ...
            <div class="flex-1 flex flex-col overflow-hidden">
                ...
                <header class="bg-card z-10 shadow-md">
                    <div class="py-4 px-6 lg:px-8 flex justify-between items-center">
                        <div class="flex items-center">
                            <button @click.stop="toggleSidebar" class="text-primary md:hidden">
                                <Bars3Icon class="h-6 w-6 stroke-current" />
                            </button>
                        </div>

                        <div class="flex items-center sm:ms-6 bg-primary relative">
                            <div class="absolute -left-3 bg-card p-2 rounded-full" />
                            <div class="absolute -right-3 bg-card p-2 rounded-full" />
                            <button @click="toggleDarkMode"
                                class="group p-2 text-white outline-dashed outline-2 outline-card bg-secondary-high">
                                <div v-if="darkMode" class="relative h-[20px] w-[20px]">
                                    <OutlineSunIcon
                ...
                <!-- Page Content -->
                <main class="custom-scrollbar flex-1 overflow-x-hidden overflow-y-auto bg-background"
                    @click="sidebarOpen = false">
                    <div class="px-8 py-8 transition-all duration-300">
                        <slot />
                    </div>
                </main>
            </div>
            ...
            <div v-if="sidebarOpen" @click="sidebarOpen = false"
                class="fixed inset-0 bg-text-reverse opacity-50 z-20 md:hidden"></div>
        </div>
    </div>
</template>
