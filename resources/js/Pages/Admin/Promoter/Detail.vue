...
<template>
    <AdminLayout title="Promoter Details">
        <PromoterDetail :promoter="promoter" @show-full-bio="showingFullBio = true"
            @show-full-address="showingFullAddress = true" @update-verification-status="updateVerificationStatus" />

        <div v-if="concerts.data && concerts.data.length > 0" class="bg-card rounded-md p-6 lg:px-20 mt-2">
            <div
                class="p-6 bg-background rounded-md grid grid-cols-3 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-6">
                <Link v-for="concert in concerts.data" :key="concert.id" :href="route('admin.concert.detail', { concert: concert.id })
                    " class="block">
        ...
        <div v-else class="bg-card rounded-md p-6 lg:px-20 mt-2">
            <div
                class="w-full flex flex-col space-y-2 font-semibold items-center text-text-medium justify-center p-6 rounded-md bg-background">
        ...
        <PromoterModal :promoter="promoter" v-model:showingFullBio="showingFullBio"
