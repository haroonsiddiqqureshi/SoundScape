        ...
        <div>
            <div class="max-w-7xl mx-auto space-y-2">
                    ...
                    <form @submit.prevent="startScraping">
                            ...
                            <div>
                                ...
                                <div class="flex space-x-6">
                                    <label class="flex items-center cursor-pointer">
                                        <input type="radio" v-model="form.artist_source" value="api" class="text-primary focus:ring-0 focus:ring-offset-0">
                                        <span class="ml-2 text-text-high">ดึงจาก API (Last.fm และ Apple Music)</span>
                                    </label>
                                    <label class="flex items-center cursor-pointer">
                                        <input type="radio" v-model="form.artist_source" value="file" class="text-primary focus:ring-0 focus:ring-offset-0">
                                        <span class="ml-2 text-text-high">อัปโหลด (.json)</span>
                                    </label>
                                </div>
                            </div>

                            <div v-if="form.artist_source === 'file'" class="p-4 rounded-md bg-background border border-background-hover">
                                <input type="file" accept=".json" @change="handleFileUpload" class="block w-full text-sm text-text-medium file:mr-2 file:px-2 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-secondary file:text-white cursor-pointer"/>
                            </div>
                        </div>
                        ...
                        <div class="mt-2">
                            ...
                            >
                                ...
                                <span v-else>ดึงข้อมูลศิลปิน</span>
                            </button>
                        </div>
                    </form>
                </div>
                ...
                <div v-if="currentJob" class="bg-card overflow-hidden shadow-sm rounded-md p-6">
                    ...
                    <div v-if="currentJob.status === 'failed'" class="bg-card rounded-md mb-4 borde-2 border-dashed border-primary">
                    ...
                    <div class="mt-6">
                        ...
                        <ul v-else class="bg-background rounded-md border border-card-hover max-h-60 overflow-y-auto p-2">
                            <li v-for="(item, index) in currentJob.results" :key="index" class="py-2 px-3 text-sm flex items-center">
