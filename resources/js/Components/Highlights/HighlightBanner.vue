...
<template>
    <div v-if="highlightsKeys.length > 0" class="relative flex text-primary-low">
        <button @click="previousHighlight"
            ...
            <ChevronDoubleLeftIcon class="w-6 h-6 sm:w-8 sm:h-8 stroke-current" />
        </button>
        <div class="shadow-md max-h-[525px] rounded-md overflow-hidden flex-1">
            <div class="flex transition-transform duration-500 ease-in-out"
                :style="{ transform: `translateX(-${currentIndex * 100}%)` }">
                <div v-for="key in highlightsKeys" :key="key" class="w-full flex-shrink-0 cursor-pointer"
                    @click="handleHighlightClick(highlights[key])">
                    <img :src="getPictureUrl(highlights[key])" :alt="highlights[key].name"
                        class="w-full h-full object-cover block" />
                </div>
            </div>
        </div>
        <button @click="nextHighlight"
            ...
            <ChevronDoubleRightIcon class="w-6 h-6 sm:w-8 sm:h-8 stroke-current" />
        </button>
        <div class="absolute -bottom-6 left-1/2 transform -translate-x-1/2 flex space-x-2">
            <button v-for="(key, index) in highlightsKeys" :key="key" @click="selectIndex(index)" :class="{
                'w-3 h-3 rounded-full bg-primary':
                    index === currentIndex,
                'w-3 h-3 rounded-full bg-primary-low':
                    index !== currentIndex,
            }" class="cursor-pointer focus:outline-none" :aria-label="`Show highlight ${index + 1}`"></button>
        </div>
    </div>
    <div v-else class="shadow-md rounded-md overflow-hidden flex-1">
        <div class="flex max-h-96">
            <div class="w-full">
                <img :src="getPictureUrl()" alt="SoundScape" class="w-full h-full object-fill block" />
            </div>
        </div>
    </div>
</template>
