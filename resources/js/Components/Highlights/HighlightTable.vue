...
const updateActiveStatus = (highlight) => {
    router.patch(route('admin.highlight.updateActiveStatus', { highlight: highlight.id }), {}, {
        preserveState: true,
        preserveScroll: true,
    });
};
...
<template>
    <div v-if="highlights && highlights.length > 0" class="bg-white shadow-md rounded-lg">
                ...
                <tr v-for="highlight in highlights" :key="highlight.id">
                    <td class="px-6 py-4 whitespace-nowrap">
                        <img
                            ...
                            @click="$emit('open-image-modal', getPictureUrl(highlight))"
                        />
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                        <button
                            ...
                            :class="highlight.is_active ? 'bg-green-600' : 'bg-gray-200'"
                        >
                            <span
                                aria-hidden="true"
                                class="pointer-events-none inline-block h-5 w-5 rounded-full bg-white shadow transform ring-0 transition ease-in-out duration-200"
                                :class="{ 'translate-x-5': highlight.is_active, 'translate-x-0': !highlight.is_active }"
                            ></span>
                        </button>
                    </td>
    ...
    <div v-else class="text-center py-12 bg-gray-50 rounded-lg">
        ...
        <p class="mt-1 text-sm text-gray-500">Get started by creating a new highlight.</p>
    </div>
</template>