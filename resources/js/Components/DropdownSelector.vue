    ...
    <div :class="[
        ...
        <div ref="dropdown" class="relative">
            <button type="button" @click="isOpen = !isOpen"
                ...
                <span class="block truncate" :class="{ 'font-normal text-text-medium': !selectedOption }">
                    {{ selectedOption ? selectedOption.name : placeholder }}
                </span>
                <span class="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-2">
                    <ChevronDownIcon class="h-4 w-4 text-text-medium transition-transform duration-200"
            ...
            <transition enter-active-class="transition ease-out duration-300"
                ...
                leave-active-class="transition ease-in duration-300" leave-from-class="transform opacity-100 scale-100"
                ...
                <div v-if="isOpen"
                    ...
                    <div v-if="isSearchable" class="sticky top-0 z-10 backdrop-blur-sm p-2">
                        <div class="relative">
                            <span class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3">
                                <MagnifyingGlassIcon class="h-5 w-5 text-text-medium" aria-hidden="true" />
                            </span>
                            <input type="text" v-model="searchInput" @input="
                    ...
                    <div v-if="options.length === 0 && isSearchable" class="px-4 py-2 text-text-medium text-center">
                        No results found
                    </div>
                </div>
            </transition>
